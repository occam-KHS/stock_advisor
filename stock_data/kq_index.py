...
price_kosdaq = price_data.merge(kosdaq_index['kosdaq_return'], left_index=True, right_index=True,
                                how='left')  # merge individual stock price data with KOSPI index
...
print(return_all.index.max())

def strategy_implement(start_date, end_date):
...
outcome_all = strategy_implement(decision_date2 , decision_date2)
kq_selection = outcome_all[outcome_all['select']==1][['code','name','buy_price','yhat']]
kq_selection.to_csv("C:\\Users\\shinki\\PycharmProjects\\webapp\\kq_selection_" + decision_date + ".csv")
print(kq_selection.sort_values(by='yhat', ascending=False))